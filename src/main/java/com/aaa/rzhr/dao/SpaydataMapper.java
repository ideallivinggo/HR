...
//获取员工薪资
public List<Map> Sgetygxz(@Param("cs") String cs);
//修改员工薪资
...
//获取绩效模板明细外键
    ...
    //指定员工中指定模板查看是否考核
    public List<Map> sbyempidbymb(@Param("empnumber") String empnumber, @Param("perid") String perid);
    //获取指定绩效考试员工信息
    public List<Map> sgetjxkhygmx(int id);
    //获取员工的绩效考核详情
    public List<Map> sgetygkhmx(@Param("id") int id,@Param("pid") int pid);
 //获取指定考核模板中的员工
    public Map sgetkhbymbyg(int id);
    //通过工号查询员工是否存在指定的部门
    public List<Map> sgetygbygh(@Param("gh") String gh, @Param("did") String did);
    //查询员工是否有评过的绩效
...
//添加绩效绩效考核
    ...
    //获取指定员工指定考核的已考核员工
    public List<Map> sgetyikhyg(@Param("perid") String perid, @Param("empid") String empid);
    //查询员工未打卡是否有请假
