    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                , {field: 'shenqingstate', title: '审核状态', width: 160,templet:function(d){
                    return d.shenqingstate == "4" ? "已通过": d.shenqingstate=="2" ? "驳回" : "审核中"
                }
                }
        ...
        var $ = layui.$, active = {
            reload: function(){
                ...
                table.reload('testReload', {
                    url:'L_chaxun_zhuan_a_b',
                    method:'post',
                    page: {
                        curr: 1 //重新从第 1 页开始
                    }
                    ,where: {
                        time1:$("#test1").val(),time2:$("#test2").val(),deptid:emp
                    }
                });
            }
        };
        ...
        table.on('tool(dome)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){
                layer.open({
                    ...
                    '申请时间：<span>'+data.shendate+'</span><br><br>' +
                    '申请理由：<span>'+data.shentext+'</span><br><br>' +
                    '部门：<span>'+data.deptname+'</span><br><br>' +
                    '一级审批人/审批时间：<span>'+data.pipeople+'/</span><span>'+data.shentime+'</span><br><br>' +
                    '审批理由：<span>'+data.bohui+'</span><br><br>' +
                    '总经理审批/审批时间：<span>'+data.zongshen+'/</span><span>'+data.zongshentiem+'</span><br><br>' +
                    '审批理由：<span>'+data.zongbohui+'</span><br><br>' +
                    '人事审批/审批时间：<span>'+data.renshi+'/</span><span>'+data.renshitiem+'</span><br><br></div>',
